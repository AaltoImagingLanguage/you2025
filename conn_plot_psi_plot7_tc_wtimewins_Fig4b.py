...
def plot_coh(ii, jj,threshold=1,compute=True):
    ...
    for i, (onset, offset) in enumerate(zip([-0.2], [1.1])):
        for ii, c in enumerate(event_id.values()):
            ...
            for jj in range(len(good_clusters)):
                axis.plot(times0[good_clusters[jj]], [X_RL.min()*(1.1+0.01*ii)]*len(good_clusters[jj][0]), '-', color=cmaps4[c],
                                        # markersize=2.6,
                                        lw=3.5,
                                            alpha=1,
                                            # label=cat
    ...
    plt.savefig(f'{folder1}/{method}_threshold{threshold}_{suffix}_tcs_{seed}_{target}_2tw2.pdf',
                bbox_inches='tight')    
    plt.show()
